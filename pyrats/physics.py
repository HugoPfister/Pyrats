...
def MofZ(M0, z, z0):
    """
    Return the typical mass (in units 1e12 Msun), at redshift z, of a
    halo of mass M0 (in units 1e12 Msun) at redshift z0.
    This is the integration from eq.(2) (the mean) from Fakhouri+10.
    """
    return M0 * (1 - M0**0.1 * 0.0627 *
                 ((0.11 * np.log(1 + z) - 1.11 * z) -
                  (0.11 * np.log(1 + z0) - 1.11 * z0)))**(-10)
...
def N_merger_until_z(ximin, M0, z, z0):
    ...
    result, residual = integrate.quad(
        lambda zz: (
            0.0104 * (1 + zz)**0.0993 * intxi * MofZ(M0, zz, z0)**0.133),
        z0, z)
...
def Mstar(Mh, z):
    """
    Give the stellar mass as a function of the halo mass (Mh) at a
    given redshift z following Behroozi et al - 2012 formula
    """
    a = 1. / (1 + z)
    nu = np.exp(-4 * a**2)
    ...
    f_1 = (-np.log10(10**(alpha * log_MhOverM1) + 1) +
           (delta * np.log10(1 + np.exp(log_MhOverM1))**gamma) /
           (1 + np.exp(10**(-log_MhOverM1))))
    return 10**(log_eps + log_M1 + f_1 -
                (-np.log10(2) + delta * np.log10(2)**gamma /
                 (1 + np.exp(1))))


def Mhalo(mStar, z):
    """
    Give the halo mass as a function of the stellar mass (mStar) at a
    given redshift z following Behroozi et al - 2012 formula
