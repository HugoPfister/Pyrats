...
def N_merger_until_z(ximin,M0,z,z0):
    ...
    between z and z0 < z
    """
    [intxi, residual]=integrate.quad(lambda x: x**-1.995*np.exp((x/0.00972)**0.263), ximin, 1)
    [result, residual]=integrate.quad(lambda zz: 0.0104*(1+zz)**0.0993*intxi*MofZ(M0,zz,z0)**0.133, z0, z) 
    return result
...
def Mstar(Mh,z):
    ...
    f_1=-np.log10(10**(alpha*log_MhOverM1)+1)+(delta*np.log10(1+np.exp(log_MhOverM1))**gamma)/(1+np.exp(10**(-log_MhOverM1)))
