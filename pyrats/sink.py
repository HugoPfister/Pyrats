...
def get_sinks(ds):
        ...
        if len(sink.ID)>0:
            ...
            sink['Mdot']=sink.M*(ds.arr(1,'code_mass/code_time').in_units('Msun/yr'))
            sink.age=np.copy(ds.arr(yt.utilities.lib.cosmology_time.get_ramses_ages(ds.t_frw,ds.tau_frw,ds.dtau,ds.time_simu,1./(ds.hubble_constant * 100 * 1e5 / 3.08e24)/ds['unit_t'],ds['time']-np.copy(sink.age),ds.n_frw/2,len(sink.age)), 'code_time').in_units('Myr'))
