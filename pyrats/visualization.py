...
def plot_all_snapshots(axis='z', field=('deposit','all_density'), folder='./', cbarmin=None, cbarmax=None, weight_field=('index','ones')):
    ...
    for dd in tqdm(files):
        ds=yt.load(dd)
        p=yt.ProjectionPlot(ds, axis=axis, fields=field, weight_field=weight_field, axes_unit=('Mpccm'))
...
def plot_halo_history(hnum, axis='z', field=('deposit','all_density'), folder='./', weight_field=('index','ones'), slice=False,\
    ...
    hid=int(t.trees[(t.trees.halo_ts==t.trees.halo_ts.max()) & (t.trees.halo_num==hnum)].halo_id)
    ...
    h=halos.HaloList(ds)
    if size==None:
        rvirfin=(h.halos.rvir[prog_id[-1]]*(1+ds.current_redshift) ,'Mpccm')
    else:
        rvirfin=size
    ...
    for i in tqdm(range(len(files))):
        ...
        h=halos.HaloList(ds)
        h.plot_halo(prog_id[-i-1], axis=axis,folder=path,field=field, r=rvirfin, weight_field=weight_field, cmap=cmap, limits=limits, plotsinks=plotsinks, units=units)
    return
...
def plot_bh_history(bhid, axis='z', field=('deposit','all_density'), folder='./', weight_field=('index','ones'), slice=False,\
    ...
    for i in tqdm(range(len(files))):
        ...
        if slice:
            p=yt.SlicePlot(ds, data_source=dd,axis=axis, fields=field\
            , center=c)
        else:
            p=yt.ProjectionPlot(ds, data_source=dd,axis=axis, fields=field\
