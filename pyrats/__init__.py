__all__ = ['halos', 'utils', 'trees', 'visualization', 'fields', 'physics', 'snaplist', 'sink']
...
def load(files='', stars=False, dm=False, bh=False, halo=False):
    ...
    if (halo) & (os.path.exists('./Halos/'+str(int(str(ds)[-5:]))+'/tree_bricks'+str(ds)[-3:])):
        ...
        if os.path.exists('./Galaxies/GalProp'+str(str(ds)[-6:])+'.csv'):
            ...
            tmp=pd.read_csv('./Galaxies/GalProp'+str(str(ds)[-6:])+'.csv',names=columns)
            tmp.index+=1
    ...
    if halo & bh:
        ...
        for bhid in ds.sink.ID:
            ...
            y=float(bh.y)               
            z=float(bh.z)          
            hid=((ds.halo.halos.x-x)**2+(ds.halo.halos.y-y)**2+(ds.halo.halos.z-z)**2).argmin()
            d=np.sqrt(((ds.halo.halos.x-x)**2+(ds.halo.halos.y-y)**2+(ds.halo.halos.z-z)**2)[hid])
            if d*L < 0.05*ds.halo.halos.rvir[hid]:    
                ...
                ds.sink.loc[ds.sink.ID==bhid,'hid']=float(ds.halo.halos.ID[hid])
