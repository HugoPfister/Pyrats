...
def dm(pfilter, data):
    try:
        data['particle_age']
        filter = np.logical_and((data["particle_identifier"] > 0) & (data["particle_age"]==data['particle_age'].min()), data["particle_age"]!= None)
    except:
        filter = data["particle_identifier"] > 0
    return filter

def young_stars(pfilter, data):
    filter = np.logical_and((data["particle_age"] > 0) & (data["particle_age"] < d.ds.arr(10, 'Myr')), data["particle_age"]!= None)
    return filter
